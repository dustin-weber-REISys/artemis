...
[`argocd/bootstrap`](../argocd/bootstrap), and each cluster's Workload Cell
catalog is authoritative in [`argocd/topology`](../argocd/topology). Chart
...
[`environments`](../environments). Put approved Workload Cell identity, sizing,
Profile selection, and typed feature choices in the matching cluster catalog.
Put reusable Artemis capabilities in an approved Workload Cell Profile. Put
pair-owned listeners, identity Secret references, destinations, authorization,
and client sources in the cell's required file under [`workloads`](../workloads).
