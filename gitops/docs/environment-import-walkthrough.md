...
3. patch `composition.artemis.apache.org/git-revision` on the rendered
   `messaging-platform` AppProject with the same approved branch, tag, or
   commit selected by the root entry. Kustomize propagates it to every child
...
The ZooKeeper Kustomize name prefix and the Artemis connection template must
continue to resolve to the same client Service. Coordination IDs and Curator
namespaces must remain unique per Workload Cell. These invariants are checked
by repository validation; do not create parallel standalone Applications for
workloads already generated by the local ApplicationSet.

### 3. Supply environment and workload values

Put cluster integrations in the matching file under
[`environments`](../environments). Put the complete Workload Cell identity,
storage and resource sizing, Profile selection, typed feature choices, and
...
- Workload Cell-unique HA and ZooKeeper identities;
- unique workload namespace, service, console, Keycloak, and Vault
  identities;
- actual NetworkPolicy namespace/pod selectors, approved internal client CIDRs,
  and approved external egress;
- declarative application addresses, queues, redelivery, dead-letter, expiry,
  paging, and auto-creation policy; and
...
### 4. Defer external Vault authentication integration

Internal and batch messaging clients do not authenticate to Artemis and do not
require a Vault-to-broker identity bridge. Complete the following only before
enabling a future external Workload Cell.
...
8. Preview the local workload ApplicationSet, change `enabled` to `"true"` for
   one test Workload Cell, allow it to reconcile, and complete the first-workload
   checks below.
...
- an external Workload Cell is enabled while its Vault-to-Artemis
  authentication bridge or injector token flow is unproven;
