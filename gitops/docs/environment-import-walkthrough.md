...
[`argocd/bootstrap`](../argocd/bootstrap). Stable Workload Cell facts are
authoritative in
...
[`environments`](../environments). Put approved Workload Cell identity, normal
storage sizing, and Profile selection in the shared Workload Cell baseline.
Put environment resource sizing, typed feature choices, and enablement in the
matching topology overlay, then regenerate the effective catalogs. Put reusable
Artemis capabilities in an approved Workload Cell Profile. Put pair-owned
listeners, identity Secret references, destinations, authorization, and client
sources in the cell's required file under [`workloads`](../workloads). The
validator rejects any other per-cell path. Raw Helm parameters and release
