...
command -v helm >/dev/null 2>&1 || {
  printf '%s\n' 'helm is required to compare the local chart render with the live broker CR' >&2
  exit 2
}
...
for enabled_pair in "${enabled_pairs[@]}"; do
  ...
  application_sync_status=$(yq -r '.status.sync.status // ""' \
    <<<"$application_json")
  application_health_status=$(yq -r '.status.health.status // ""' \
    <<<"$application_json")
  application_reconciled_revision=$(yq -r '.status.sync.revision // ""' \
    <<<"$application_json")
  [[ "$application_sync_status" == Synced ]] || \
    error "Application $argocd_namespace/$application sync status is $application_sync_status; expected Synced"
  [[ "$application_health_status" == Healthy ]] || \
    error "Application $argocd_namespace/$application health is $application_health_status; expected Healthy"
  if [[ -n "$checkout_revision" && -n "$application_reconciled_revision" && \
        "$application_reconciled_revision" != "$checkout_revision" ]]; then
    error "Application $argocd_namespace/$application reconciled revision $application_reconciled_revision differs from repository checkout $checkout_revision"
  fi
  printf 'Artemis Application: %s/%s revision=%s\n' \
    "$application_sync_status" "$application_health_status" \
    "${application_reconciled_revision:-unknown}"
  ...
  # Render the live child Application's exact Helm inputs against the checkout.
  # This separates a repository chart defect from a stale/different manifest
  # generated by Argo CD at the same reported Git revision.
  chart_dir="$repo_root/${actual_path#gitops/}"
  helm_value_args=()
  while IFS= read -r value_file; do
    [[ -n "$value_file" ]] || continue
    helm_value_args+=(--values "$chart_dir/$value_file")
  done < <(yq -r '.spec.source.helm.valueFiles[]?' <<<"$application_json")
  helm_parameter_args=()
  while IFS=$'\t' read -r parameter_name parameter_value force_string; do
    [[ -n "$parameter_name" ]] || continue
    if [[ "$force_string" == true ]]; then
      helm_parameter_args+=(--set-string "$parameter_name=$parameter_value")
    else
      helm_parameter_args+=(--set "$parameter_name=$parameter_value")
    fi
  done < <(yq -r '
    .spec.source.helm.parameters[]?
    | [.name, (.value // ""), (.forceString // false)]
    | @tsv
  ' <<<"$application_json")
  rendered_workload=
  rendered_resource_template_complete=false
  if ! rendered_workload=$(helm template "$application" "$chart_dir" \
    --namespace "$workload_namespace" \
    "${helm_value_args[@]}" \
    "${helm_parameter_args[@]}"); then
    error "Local Helm render failed for Application $application"
  else
    rendered_broker_count=$(BROKER_CR="$broker_cr" yq -r \
    ...
    duplicate_resource_identities=$(RELEASE_NAMESPACE="$workload_namespace" yq -r '
      ...
      | @tsv
    ' <<<"$rendered_workload" | sort | uniq -d)
    if [[ -n "$duplicate_resource_identities" ]]; then
      error "Local Helm render for Application $application contains duplicate resource identities: $(tr '\n' ';' <<<"$duplicate_resource_identities")"
    fi

    rendered_missing_template_labels=()
    for required_label in "${required_resource_labels[@]}"; do
      matching_template_count=$(BROKER_CR="$broker_cr" LABEL="$required_label" yq -r '
        select(.kind == "ActiveMQArtemis" and .metadata.name == strenv(BROKER_CR))
      ...
      if [[ "$matching_template_count" -eq 0 ]]; then
        rendered_missing_template_labels+=("$required_label")
      fi
    done
    if ((${#rendered_missing_template_labels[@]})); then
      error "Local Helm render for Application $application has no unscoped resourceTemplate carrying required labels: $(IFS=,; printf '%s' "${rendered_missing_template_labels[*]}")"
    elif [[ "$rendered_broker_count" -eq 1 && -z "$duplicate_resource_identities" ]]; then
      rendered_resource_template_complete=true
      printf 'Local chart render: one ActiveMQArtemis, no duplicate identities, required resourceTemplate labels present\n'
    fi
  fi
  ...
  if ((${#missing_resource_template_labels[@]})); then
    ...
    if [[ "$rendered_resource_template_complete" == true && \
          -n "$checkout_revision" && \
          "$application_reconciled_revision" == "$checkout_revision" ]]; then
      error "ActiveMQArtemis $workload_namespace/$broker_cr differs from the valid local Helm render at the same revision $checkout_revision; inspect Argo CD's generated manifests and request a hard refresh through the authorized workflow"
    fi
  fi
